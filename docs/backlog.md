...
**Статус: в работе.** Программный срез и последние UX-корректировки реализованы; локальная acceptance подтверждена владельцем. Production и HTTPS развёрнуты; внешняя acceptance guest/login/logout и обоих разделов подтверждена владельцем. Завершается канонизация URL /collection и /wishlist. Подробности — [auth и acceptance](stage-2-auth.md).
